#!/usr/bin/env bash
# chat.sh — Send a message to KKOCLAW and collect the streaming response.
#
# Usage:
...
#   KKOCLAW_URL=http://host:9191 bash chat.sh "hi"    # custom endpoint
#
# Environment variables:
#   KKOCLAW_URL           — Unified proxy base URL (default: http://localhost:9191)
#   KKOCLAW_GATEWAY_URL   — Gateway API base URL (default: $KKOCLAW_URL)
#   KKOCLAW_LANGGRAPH_URL — LangGraph API base URL (default: $KKOCLAW_URL/api/langgraph)
#
...
GATEWAY_URL="${KKOCLAW_GATEWAY_URL:-$KKOCLAW_URL}"
LANGGRAPH_URL="${KKOCLAW_LANGGRAPH_URL:-$KKOCLAW_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: KKOCLAW is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure KKOCLAW is running. Start it with: cd <kkoclaw-dir> && make dev" >&2
  exit 1
fi
...
gateway_url = sys.argv[2].rstrip("/") if len(sys.argv) > 2 else "http://localhost:9191"
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from KKOCLAW: {data_str}", file=sys.stderr)
            sys.exit(1)
