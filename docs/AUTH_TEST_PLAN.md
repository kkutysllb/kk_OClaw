...
> - `KKOCLAW_HOME` 挂载到宿主机目录（持久化 `users.db`）
...
# 检查宿主机上的 users.db
ls -la ${KKOCLAW_HOME:-backend/.kkoclaw}/users.db
sqlite3 ${KKOCLAW_HOME:-backend/.kkoclaw}/users.db \
  "SELECT email FROM users WHERE email='docker-test@example.com';"
```

**预期：** users.db 在宿主机 `KKOCLAW_HOME` 目录中，查询可见刚注册的用户。
...
```bash
# 凭证文件写在挂载到宿主机的 KKOCLAW_HOME 下
ls -la ${KKOCLAW_HOME:-backend/.kkoclaw}/admin_initial_credentials.txt
# 预期文件权限: -rw------- (0600)

cat ${KKOCLAW_HOME:-backend/.kkoclaw}/admin_initial_credentials.txt
...
**预期：**
- 凭证文件存在于 `KKOCLAW_HOME` 下，权限 `0600`
