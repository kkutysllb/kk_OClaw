...
| | `langchain.create_agent` | `make_lead_agent` | `KKOCLAWClient`（增强后） |
|---|---|---|---|
...
| 适合谁 | 写 LangChain 的人 | 内部使用 | **所有 KKOCLAW 用户** |
...
```
    ┌──────────────────────┐
    │   KKOCLAWClient     │  ← 唯一公开 API（chat/stream + 管理）
    └──────────┬───────────┘
    ┌──────────▼───────────┐
...
`KKOCLAWClient` 是唯一公开 API。`create_kkoclaw_agent` 和 `make_lead_agent` 都是内部实现。

用户通过 `KKOCLAWClient` 三个参数控制行为：
...
- **向前兼容** — 现有 `KKOCLAWClient()` 无参构造行为不变
- **harness 边界合规** — 不 import `app.*`（`test_harness_boundary.py` 强制）

## 3. API 设计

### 3.1 `KKOCLAWClient` — 唯一公开 API
...
# 用法 2：只改参数，不换实现
client = KKOCLAWClient(config={"memory": {"max_facts": 50}})

# 用法 3：替换 middleware 实现
client = KKOCLAWClient(features=RuntimeFeatures(auto_title=MyTitleMiddleware()))

# 用法 4：添加自定义 middleware
client = KKOCLAWClient(extra_middleware=[MyAuditMiddleware()])

# 用法 5：纯 SDK（无 config.yaml）
client = KKOCLAWClient(config={
...
client = KKOCLAWClient(features=RuntimeFeatures(memory=MyMemoryMiddleware()))
...
Phase 1（当前 PR #1203）:
  ...
  ✓ 不改 KKOCLAWClient 和 make_lead_agent
  ✗ middleware 内部仍读 config（已知限制）

Phase 2（#1380）:
  - KKOCLAWClient 构造函数增加可选参数（model, tools, features, system_prompt）
...
| 问题 | 决议 | 理由 |
|------|------|------|
| 公开 API | `KKOCLAWClient` 唯一入口 | 自顶向下，先改现有 API 再抽底层 |
