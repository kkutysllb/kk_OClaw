...
#
# This script MUST be run before `cargo tauri build`. It:
#   1. Runs PyInstaller from backend/ using oclaw-gateway.spec
#   2. Copies the output directory to desktop/src-tauri/resources/gateway/
#
# Tauri then bundles everything in resources/gateway/ into the final .app/.dmg.
# Without this step the app is ~53 MB (frontend only) and the gateway fails
# to start at runtime.
#
set -euo pipefail
...
RESOURCES_DIR="$DESKTOP_DIR/src-tauri/resources/gateway"
...
[[ -f "$PYINSTALLER_OUTPUT/oclaw-gateway" ]] || fail "Gateway executable not found in output"
...
echo ""
echo "=========================================="
ok "Gateway bundle ready for Tauri packaging"
echo "=========================================="
echo ""
