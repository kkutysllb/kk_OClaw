...
def _make_app_config(models: list[ModelConfig]) -> AppConfig:
    return AppConfig(
        models=models,
        sandbox=SandboxConfig(use="kkoclaw.sandbox.local:LocalSandboxProvider"),
    )
...
def test_internal_make_lead_agent_uses_explicit_app_config(monkeypatch):
    ...
    import kkoclaw.tools as tools_module
...
def test_create_summarization_middleware_uses_configured_model_alias(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "get_app_config", _raise_get_app_config)
    monkeypatch.setattr(lead_agent_module, "create_chat_model", _fake_create_chat_model)
    monkeypatch.setattr(lead_agent_module, "KKOCLAWSummarizationMiddleware", lambda **kwargs: kwargs)
