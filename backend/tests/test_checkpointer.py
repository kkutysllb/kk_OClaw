...
import kkoclaw.config.app_config as app_config_module
from kkoclaw.config.checkpointer_config import (
    CheckpointerConfig,
    get_checkpointer_config,
    load_checkpointer_config_from_dict,
    set_checkpointer_config,
)
from kkoclaw.runtime.checkpointer import get_checkpointer, reset_checkpointer
...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("kkoclaw.runtime.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
    ...
    def test_sqlite_creates_parent_dir(self):
        ...
        .kkoclaw directory has not been created).
        """
...
# ---------------------------------------------------------------------------
# KKOCLAWClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """KKOCLAWClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            patch("kkoclaw.client.get_app_config", return_value=config_mock),
            patch("kkoclaw.client.create_agent", side_effect=fake_create_agent),
            patch("kkoclaw.client.create_chat_model", return_value=MagicMock()),
            patch("kkoclaw.client._build_middlewares", return_value=[]),
            patch("kkoclaw.client.apply_prompt_template", return_value=""),
            patch("kkoclaw.client.KKOCLAWClient._get_tools", return_value=[]),
        ):
            client = KKOCLAWClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = KKOCLAWClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
