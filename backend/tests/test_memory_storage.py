...
class TestFileMemoryStorage:
    ...
    def test_get_memory_file_path_global(self, tmp_path):
        ...
        with patch("kkoclaw.agents.memory.storage.get_paths", side_effect=mock_get_paths):
            with patch("kkoclaw.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_path="")):
    ...
    @pytest.mark.parametrize("valid_name", ["agent_name", "coding_agent", "agent-name"])
    def test_validate_agent_name_allows_configured_agent_pattern(self, valid_name):
        """Memory storage should accept the same agent names as the agent config layer."""
        storage = FileMemoryStorage()

        storage._validate_agent_name(valid_name)
...
class TestGetMemoryStorage:
    ...
    def reset_storage_instance(self):
        ...
        import kkoclaw.agents.memory.storage as storage_mod
    ...
    def test_returns_file_memory_storage_by_default(self):
        ...
        with patch("kkoclaw.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="kkoclaw.agents.memory.storage.FileMemoryStorage")):
    ...
    def test_falls_back_to_file_memory_storage_on_error(self):
        ...
        with patch("kkoclaw.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="non.existent.StorageClass")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
    ...
    def test_get_memory_storage_invalid_class_fallback(self):
        ...
        with patch("kkoclaw.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="os.path.join")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
    ...
    def test_get_memory_storage_non_subclass_fallback(self):
        ...
        with patch("kkoclaw.agents.memory.storage.get_memory_config", return_value=MemoryConfig(storage_class="builtins.dict")):
            storage = get_memory_storage()
            assert isinstance(storage, FileMemoryStorage)
