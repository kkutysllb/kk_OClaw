...
kkoclaw.subagents.executor. This test file uses delayed import via fixture to test
...
def _setup_executor_classes():
    ...
    original_executor = sys.modules.get("kkoclaw.subagents.executor")

    # Remove mocked executor if exists (from conftest.py)
    if "kkoclaw.subagents.executor" in sys.modules:
        del sys.modules["kkoclaw.subagents.executor"]
    ...
    if original_executor is not None:
        sys.modules["kkoclaw.subagents.executor"] = original_executor
    elif "kkoclaw.subagents.executor" in sys.modules:
        del sys.modules["kkoclaw.subagents.executor"]
...
class TestAgentConstruction:
    ...
    def test_create_agent_threads_explicit_app_config_to_model_and_middlewares(
        ...
        import kkoclaw.config as config_module
        from kkoclaw.subagents import executor as executor_module
        ...
        monkeypatch.setitem(
            sys.modules,
            "kkoclaw.agents.middlewares.tool_error_handling_middleware",
            _module(
                "kkoclaw.agents.middlewares.tool_error_handling_middleware",
    ...
    async def test_load_skill_messages_uses_explicit_app_config_for_skill_storage(
        ...
        monkeypatch.setattr("kkoclaw.skills.storage.get_or_new_skill_storage", fake_get_or_new_skill_storage)
...
class TestCleanupBackgroundTask:
    ...
    def executor_module(self, _setup_executor_classes):
        ...
        from kkoclaw.subagents import executor
