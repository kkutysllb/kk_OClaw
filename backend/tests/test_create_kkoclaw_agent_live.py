"""Live integration tests for create_kkoclaw_agent.
...
def test_minimal_agent_responds():
    """create_kkoclaw_agent(model) produces a graph that returns a response."""
    from kkoclaw.agents.factory import create_kkoclaw_agent

    model = _make_model()
    graph = create_kkoclaw_agent(model, features=None, middleware=[])
...
def test_agent_with_custom_tool():
    ...
    model = _make_model()
    graph = create_kkoclaw_agent(model, tools=[add], middleware=[])
...
def test_features_mode_middleware_chain():
    ...
    graph = create_kkoclaw_agent(model, features=feat)
