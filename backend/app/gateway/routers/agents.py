...
from kkoclaw.config.agents_api_config import get_agents_api_config
from kkoclaw.config.agents_config import AgentConfig, list_custom_agents, load_agent_config, load_agent_soul
...
async def update_agent(name: str, request: AgentUpdateRequest) -> AgentResponse:
    ...
    if not agent_dir.exists() and paths.agent_dir(name).exists():
        raise HTTPException(
            status_code=409,
            detail=(f"Agent '{name}' only exists in the legacy shared layout and is not scoped to a user. "
...
async def delete_agent(name: str) -> None:
    ...
    if paths.agent_dir(name).exists():
        raise HTTPException(
            status_code=409,
            detail=(f"Agent '{name}' only exists in the legacy shared layout and is not scoped to a user. "
                    f"Run scripts/migrate_user_isolation.py to move legacy agents into the per-user layout before deleting."),
        )
