# Contributing to KKOCLAW Backend

Thank you for your interest in contributing to KKOCLAW! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/kkoclaw.git
   cd kkoclaw
   ```
...
```python
import pytest
from kkoclaw.models.factory import create_chat_model
...
1. Create tool in `packages/harness/kkoclaw/tools/builtins/` or `packages/harness/kkoclaw/community/`:

```python
# packages/harness/kkoclaw/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: kkoclaw.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/kkoclaw/agents/middlewares/`:

```python
# packages/harness/kkoclaw/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/kkoclaw/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/kkoclaw/config/app_config.py` with new fields
...
Thank you for contributing to KKOCLAW!
