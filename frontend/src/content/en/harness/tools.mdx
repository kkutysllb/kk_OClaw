...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. KKOCLAW provides built-in
...
KKOCLAW organizes tools into four categories:
...
tools:
  ...
  - use: kkoclaw.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
Install: `cd backend && uv add 'kkoclaw-harness[tavily]'`
...
Install: `cd backend && uv add 'kkoclaw-harness[exa]'`
...
Install: `cd backend && uv add 'kkoclaw-harness[firecrawl]'`
...
tools:
  ...
  # Or use InfoQuest:
  # - use: kkoclaw.community.infoquest.tools:image_search_tool
  #   api_key: $INFOQUEST_API_KEY
```
