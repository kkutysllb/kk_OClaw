...
- Check that the storage path is writable: `ls -la backend/.kkoclaw/`.
- Look for memory update errors in `logs/langgraph.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.kkoclaw/`:
...
In Docker deployments, the bind-mounted host path (`$KKOCLAW_ROOT/backend/.kkoclaw/`) is the source of truth — back up the host path.
