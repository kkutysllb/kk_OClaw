...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.kkoclaw/threads/
```

**解决**：

1. 确保 `backend/.kkoclaw/` 对运行 KKOCLAW 的进程可写。
...
- **线程数据**：`backend/.kkoclaw/threads/` — 每个线程的上传文件、输出和工作区文件
- **检查点**：取决于检查点器配置（SQLite：`backend/.kkoclaw/checkpoints.db`，Redis：外部存储）
- **记忆**：`backend/.kkoclaw/memory.json`（以及 `agents/*/memory.json`）
